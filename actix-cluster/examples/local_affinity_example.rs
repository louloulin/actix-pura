...
async fn main() -> std::io::Result<()> {
    ...
    // 创建并启动集群系统
    let mut cluster = ClusterSystem::new("local-affinity-demo", config);
    ...
    let primary_path = primary.actor_path();
    ...
    println!("Started secondary actor at path: {}", secondary.actor_path());
