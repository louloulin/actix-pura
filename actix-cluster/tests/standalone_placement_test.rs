...
fn test_round_robin_placement_strategy() {
    ...
    let unique_nodes: HashSet<_> = nodes.iter().cloned().collect();
...
fn test_redundant_placement_strategy() {
    ...
    // We should have used at least 2 of the 5 nodes (statistically very likely)
    // This is a more lenient assertion than requiring 3+ nodes
    assert!(selected_nodes.len() >= 2, 
            "Expected at least 2 different nodes to be selected, got {}", 
            selected_nodes.len());
}
