...
pub fn get_connector<T: ?Sized + Any>(name: &str) -> Option<ConnectorFactory<T>> {
    ...
    if let Some(factory) = registry.get::<T>(name) {
        // Clonar la fábrica para evitar referencias a la variable local
        let factory_clone = factory.clone();
        // Crear una nueva fábrica que llama a la clonada
        Some(factory_clone)
    } else {
        None
    }
...
mod tests {
    ...
    fn test_connector_registry() {
        ...
        registry.register::<dyn TestConnector>(
            "test",
            Arc::new(|config: Value| -> Result<Box<dyn TestConnector>> {
