...
use crate::actor::{
    ActorRef, ActorRegistry, MessageRouter, ActorHandler,
...
async fn test_actor_reference_system() {
    ...
    assert_eq!(result, "Processed: Hello from actor ref");
...
async fn test_registry_and_router() {
    ...
    assert_eq!(result1, "Processed: Message for actor1");
    assert_eq!(result2, "Processed: Message for actor2");
...
async fn test_cluster_actor() {
    ...
    assert!(matches!(status, dataflare_core::message::WorkflowStatus::Initialized));
    ...
    assert!(matches!(status, dataflare_core::message::WorkflowStatus::Stopped));
} 