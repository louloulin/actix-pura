...
/// WASM聚合器组件
pub struct WasmAggregatorComponent {
    config: WasmComponentConfig,
    metadata: WasmPluginMetadata,
    initialized: bool,
}

impl WasmAggregatorComponent {
    pub async fn new(config: WasmComponentConfig) -> WasmResult<Self> {
        if config.component_type != WasmComponentType::Aggregator {
            return Err(WasmError::Component("组件类型必须是Aggregator".to_string()));
        }
...
#[async_trait]
impl WasmComponent for WasmAggregatorComponent {
