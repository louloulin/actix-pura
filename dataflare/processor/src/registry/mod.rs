...
use crate::{
    ...
    join::JoinProcessor,
    dtl::DTLProcessor,
    ai::{AIEmbeddingProcessor, AIAnalysisProcessor, VectorSearchProcessor, AIRouterProcessor},
};
...
pub fn register_default_processors() {
    ...
    // 注册DTL处理器
    register_processor("dtl", Arc::new(|config: Value| -> Result<Box<dyn Processor>> {
        let processor = DTLProcessor::from_json(config)?;
        Ok(Box::new(processor))
    }));

    // 注册AI嵌入处理器
    register_processor("ai_embedding", Arc::new(|config: Value| -> Result<Box<dyn Processor>> {
        let processor = AIEmbeddingProcessor::from_json(config)?;
        Ok(Box::new(processor))
    }));

    // 注册AI分析处理器
    register_processor("ai_analysis", Arc::new(|config: Value| -> Result<Box<dyn Processor>> {
        let processor = AIAnalysisProcessor::from_json(config)?;
        Ok(Box::new(processor))
    }));

    // 注册向量搜索处理器
    register_processor("vector_search", Arc::new(|config: Value| -> Result<Box<dyn Processor>> {
        let processor = VectorSearchProcessor::from_json(config)?;
        Ok(Box::new(processor))
    }));

    // 注册AI路由处理器
    register_processor("ai_router", Arc::new(|config: Value| -> Result<Box<dyn Processor>> {
        let processor = AIRouterProcessor::from_json(config)?;
        Ok(Box::new(processor))
    }));

    // 注册WASM处理器
    #[cfg(feature = "wasm")]
    register_processor("wasm", Arc::new(|config: Value| -> Result<Box<dyn Processor>> {
